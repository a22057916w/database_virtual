...
def SQL_SELLS_TOTAL_TPE():
    ...
    sql = "CREATE TABLE sells_total_rows_TPE (id INT AUTO_INCREMENT PRIMARY KEY, post_id INT(255), addr VARCHAR(255), section VARCHAR(255), area VARCHAR(255), price VARCHAR(255), url VARCHAR(255))"
    mycursor.execute(sql)


    house_sheet = read_excel("sells/data/TPE/info/total_rows_TPE.xlsx")

    sql = "INSERT INTO sells_total_rows_TPE (post_id, addr, section, area, price, url) VALUES (%s, %s, %s, %s, %s, %s)"
