...
def save(row_data):
    os.makedirs("lease/data/NTC/info", exist_ok=True)

    df = pd.DataFrame.from_dict(row_data)
    writer = pd.ExcelWriter('lease/data/NTC/info/total_rows_NTC.xlsx', engine='xlsxwriter')
    df.to_excel(writer, sheet_name='lease_total_rows_data')
    writer.save()

    with open('lease/data/NTC/info/total_rows_NTC.json', 'w', encoding='utf-8') as f:
